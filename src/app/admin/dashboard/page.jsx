...
export default function AdminDashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fffaf8] text-[#8C1E1E]">
      <aside className="fixed left-0 top-0 h-screen w-80 bg-[#8C1E1E] text-white">
            ...
            {[
              ...
              <div
                ...
                className={`flex cursor-pointer items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition ${
                  selectedNav === item
                    ? "bg-white text-[#8C1E1E] shadow-md"
                    : "text-white/80 hover:bg-white/10 hover:text-white"
                }`}
              >
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-white/20 bg-white/5">
                  {index + 1}
                </span>
                <span>{item}</span>
              </div>
        ...
        <div className="absolute bottom-4 left-5 right-5 rounded-2xl border border-white/20 bg-white/10 px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full bg-white px-3 py-1 text-[10px] font-black text-[#8C1E1E]">
              Online
            </span>
          </div>
      ...
      <section className="ml-80 min-h-screen">
        <header className="flex h-20 items-center justify-between border-b border-[#8C1E1E]/20 bg-white px-8">
          <div className="flex items-center gap-4">
            <span className="text-2xl font-black text-[#8C1E1E]">{selectedNav}</span>
          </div>
          <div className="flex items-center gap-4">
            <button
              type="button"
              onClick={handleLogout}
              className="rounded-xl border border-[#8C1E1E]/30 px-4 py-2 cursor-pointer text-sm font-bold text-[#8C1E1E] hover:bg-[#8C1E1E] hover:text-white"
            >
              Logout
            </button>
          </div>
        ...
        <div className="px-8 py-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-black text-[#8C1E1E]">Good Morning, Admin</h1>
              <p className="mt-1 text-sm font-medium text-slate-500">
                Here’s what’s happening your InviteArc platform today.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <span className="rounded-full bg-white px-4 py-2 text-xs font-black uppercase tracking-wide text-[#8C1E1E] shadow-sm border border-[#8C1E1E]/20">
          ...
          {selectedNav === "Dashboard" ? (
            <>
              <section className="grid gap-4 md:grid-cols-4">
                {summaryCards.map((item) => (
                  <article key={item.label} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-6">
                      <div className="text-sm font-bold uppercase text-slate-500">{item.label}</div>
                      <div className="mt-2 text-4xl font-black text-slate-950">{item.value}</div>
                      <div className="mt-2 text-xs font-semibold text-slate-500">{item.meta}</div>
                    </div>
                  </article>
                ))}
              </section>
              ...
              <section className="mt-6 grid gap-6 lg:grid-cols-[2fr_1fr]">
                <article className="rounded-3xl border border-[#8C1E1E]/20 bg-white p-6 shadow-sm ">
                  <div className="mb-4 flex items-center justify-between ">
                    <div>
                      <span className="text-lg font-black text-[#8C1E1E]">Sales Overview</span>
                      <span className="ml-2 text-xs font-semibold text-slate-400">Revenue from template sales</span>
                    </div>
                    <div className="flex gap-2">
                      <button className="rounded-xl border border-[#8C1E1E]/20 px-3 py-2 text-xs font-bold text-[#8C1E1E]">This Month</button>
                    </div>
                  </div>

                  <div className="rounded-3xl border border-[#8C1E1E]/10 bg-[#8C1E1E]/5 p-5 h-[87%]">
                        ...
                        return (
                          <div key={`${bar.label}-${index}`} className="flex flex-1 flex-col items-center justify-end gap-2">
                            <div className="flex h-full w-full items-end justify-center">
                              <div className="w-12 rounded-t-2xl bg-[#8C1E1E] shadow-md" title={bar.value} style={{ height: `${barHeight}px` }}></div>
                            </div>
                ...
                <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <span className="text-lg font-black text-slate-900">Recent Orders</span>
                    <button className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-bold text-white">View All →</button>
                  </div>
              ...
              <section className="mt-6 grid gap-6 lg:grid-cols-[2fr_1fr]">
                <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <span className="text-lg font-black text-slate-900">Popular Templates</span>
                    <button className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-bold text-white">View All →</button>
                  </div>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    {(stats.popularTemplates || []).map((template) => (
                      <div key={template.title} className="rounded-2xl border border-slate-100 bg-slate-50 p-3">
                        <div className="h-28 rounded-xl border border-slate-200 bg-cover bg-center" style={{ backgroundImage: template.previewImage ? `url(${template.previewImage})` : "none" }}></div>
                        <div className="mt-3">
                ...
                <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <span className="text-lg font-black text-slate-900">Recent Activity</span>
                    <button className="rounded-xl bg-slate-950 px-3 py-2 text-xs font-bold text-white">Refresh</button>
                  </div>
          ...
          {selectedNav === "Users" ? (
            <section className="mt-6 grid gap-4 lg:grid-cols-[360px_1fr]">
              <aside className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <span className="text-lg font-black text-slate-900">Users</span>
                  ...
                  {(stats.users || []).map((user) => (
                    ...
                    >
                      <span className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-black ${
                        selectedUser?.id === user.id ? "bg-white text-slate-950" : "bg-slate-950 text-white"
                      }`}>
                ...
                {selectedUser ? (
                  <div>
                    <div className="mb-6 flex items-center gap-4">
                      <span className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-950 text-xl font-black text-white">
                        {String(selectedUser.name || "U").slice(0, 2).toUpperCase()}
                      </span>
                      <div>
                        <span className="text-xs font-black uppercase tracking-wide text-slate-500">User Profile</span>
                        <h3 className="mt-2 text-2xl font-black text-slate-900">{selectedUser.name}</h3>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        <div className="text-[11px] font-black uppercase text-slate-500">Mobile Number</div>
                        <div className="mt-2 text-sm font-black text-slate-900">{selectedUser.mobileNumber}</div>
                      </div>
                      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        <div className="text-[11px] font-black uppercase text-slate-500">Role</div>
                        <div className="mt-2 text-sm font-black text-slate-900 capitalize">{selectedUser.role}</div>
                      </div>
                      <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                        <div className="text-[11px] font-black uppercase text-slate-500">Joined</div>
          ...
          {selectedNav === "Payments" ? (
            <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <span className="text-lg font-black text-slate-900">Payments</span>
                  <span className="ml-2 text-xs font-semibold text-slate-400">Payment details</span>
                </div>
                <button className="rounded-xl bg-slate-950 px-4 py-2 text-xs font-bold text-white">Export</button>
              </div>

              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase text-slate-500">Total Revenue</div>
                  <div className="mt-2 text-3xl font-black text-slate-900">{stats.totals.revenueDisplay}</div>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase text-slate-500">Total Orders</div>
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  ...
                  <div className="mt-2 text-xl font-black text-emerald-700">Online</div>
                </div>
              </div>

              <div className="mt-6">
                <div className="mb-3 text-sm font-black uppercase text-slate-500">Recent Payment Records</div>
                <div className="space-y-3">
                  {(stats.recentOrders || []).map((order, index) => (
                    <div key={order.id} className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-4">
                      <div className="flex items-center gap-3">
                        <span className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-950 text-xs font-black text-white">
                          {index + 1}
                        </span>
                        <div>
                          <div className="text-sm font-black text-slate-900">{order.templateTitle}</div>
          ...
          {selectedNav === "Settings" ? (
            <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase text-slate-500">Template Engine</div>
                  <div className="mt-2 text-sm font-black text-slate-900">{admin.name}</div>
                </div>
